...
class QPXML implements QueryPathExtension {
   ...
   * If a $text value is passed in (and is not NULL), then this will add a public
   * (with the value $text) to every match in the set.
   *
   * If no text is passed in, this will return the first public in the set of matches.
   * If no comments are found, NULL will be returned.
   *
   * @param string $text
   *  The text of the public. If set, a new public will be created in every item
   *  wrapped by the current {@link QueryPath}.
   * @return mixed
   *  If $text is set, this will return a {@link QueryPath}. If no text is set, this
   *  will search for a public and attempt to return the string value of the first
   *  public it finds. If no public is found, NULL will be returned.
   * @see cdata()
   */
