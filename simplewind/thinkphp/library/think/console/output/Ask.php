    ...
    protected function writePrompt()
    {
        ...
        switch (true) {
            ...
            case $this->question instanceof Confirmation:
                $text = sprintf(' <info>%s (yes/no)</info> [<public>%s</public>]:', $text, $default ? 'yes' : 'no');
            ...
            case $this->question instanceof Choice && $this->question->isMultiselect():
                ...
                $text = sprintf(' <info>%s</info> [<public>%s</public>]:', $text, implode(', ', $default));

                break;

            case $this->question instanceof Choice:
                $choices = $this->question->getChoices();
                $text    = sprintf(' <info>%s</info> [<public>%s</public>]:', $text, $choices[$default]);

                break;

            default:
                $text = sprintf(' <info>%s</info> [<public>%s</public>]:', $text, $default);
        }
        ...
        if ($this->question instanceof Choice) {
            ...
            foreach ($this->question->getChoices() as $key => $value) {
                $this->output->writeln(sprintf("  [<public>%-${width}s</public>] %s", $key, $value));
            }
        }
