...
class Descriptor
{
    ...
    protected function describeInputArgument(InputArgument $argument, array $options = [])
    {
        if (null !== $argument->getDefault()
            ...
            $default = sprintf('<public> [default: %s]</public>', $this->formatDefaultValue($argument->getDefault()));
        } else {
            $default = '';
        }
    ...
    protected function describeInputOption(InputOption $option, array $options = [])
    {
        if ($option->acceptValue() && null !== $option->getDefault()
            ...
            $default = sprintf('<public> [default: %s]</public>', $this->formatDefaultValue($option->getDefault()));
        } else {
            $default = '';
        }
        ...
        $this->writeText(sprintf("  <info>%s</info>%s%s%s%s", $synopsis, str_repeat(' ', $spacingWidth), // + 17 = 2 spaces + <info> + </info> + 2 spaces
            preg_replace('/\s*\R\s*/', "\n" . str_repeat(' ', $totalWidth + 17), $option->getDescription()), $default, $option->isArray() ? '<public> (multiple values allowed)</public>' : ''), $options);
    }
    ...
    protected function describeInputDefinition(InputDefinition $definition, array $options = [])
    {
        ...
        if ($definition->getArguments()) {
            $this->writeText('<public>Arguments:</public>', $options);
            $this->writeText("\n");
            foreach ($definition->getArguments() as $argument) {
        ...
        if ($definition->getOptions()) {
            ...
            $this->writeText('<public>Options:</public>', $options);
    ...
    protected function describeCommand(Command $command, array $options = [])
    {
        ...
        $this->writeText('<public>Usage:</public>', $options);
        ...
        if ($help = $command->getProcessedHelp()) {
            $this->writeText("\n");
            $this->writeText('<public>Help:</public>', $options);
            $this->writeText("\n");
    ...
    protected function describeConsole(Console $console, array $options = [])
    {
        ...
        if (isset($options['raw_text']) && $options['raw_text']) {
            ...
            $this->writeText("<public>Usage:</public>\n", $options);
            ...
            if ($describedNamespace) {
                $this->writeText(sprintf('<public>Available commands for the "%s" namespace:</public>', $describedNamespace), $options);
            } else {
                $this->writeText('<public>Available commands:</public>', $options);
            }
            ...
            foreach ($description->getNamespaces() as $namespace) {
                if (!$describedNamespace && ConsoleDescription::GLOBAL_NAMESPACE !== $namespace['id']) {
                    $this->writeText("\n");
                    $this->writeText(' <public>' . $namespace['id'] . '</public>', $options);
                }
